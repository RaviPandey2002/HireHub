...
async function CreateJobApplicationAction(data: unknown, pathToRevalidate: string) {
    try {
        ...
        // Verify that the job opening exists and is actively accepting applications
        const job = await db.jobs.findUnique({
            where: { id: parsed.data.jobId },
            select: { id: true, status: true, title: true, companyName: true },
        });
        ...
        if (job.status === "Closed") {
            return { error: "This job opening is closed and is no longer accepting applications." };
        }
